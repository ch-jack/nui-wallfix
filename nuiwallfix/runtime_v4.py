"""Runtime v4: strict SRI and transparent target/state write counts."""
...
def _strict_sri_matches(data, integrity):
    ...
    if not tokens:
        return False
    strongest = max(rank[algorithm] for algorithm, _expected in tokens)
...
_api_apply_base = _v1.api_apply


def api_apply(*args, **kwargs):
    result = _api_apply_base(*args, **kwargs)
    if result.get("status") != "applied":
        return result
    target_files = result["summary"].get("written_files", 0)
    state_files = 1  # run.json
    try:
